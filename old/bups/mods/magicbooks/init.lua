...
function prepare_map(position)
	local minp1=vector.multiply(position, {x=80,y=80,z=180})
	local maxp2=vector.add(minp1,{x=80,y=80,z=180})
	local v = VoxelManip()
	local minp, maxp = v:read_from_map(minp1, maxp2)
	minp=vector.add(minp,{x=0,y=80,z=0})
	maxp=vector.add(maxp,{x=0,y=80,z=0})
	local vm = VoxelManip()
	local read_data = vm:get_data()
	minp, maxp = vm:read_from_map(minp, maxp)
	vm:set_data(read_data)
